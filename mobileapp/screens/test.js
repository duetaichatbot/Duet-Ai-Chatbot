...
//     let response = await axios.post("http://localhost:8000/message", {
//       query: newMessage,
//     });
  ...
  return (
    <Box>
      <div id="messageWindow" className="messageWindow">
        {myMessages.map((eachMessage, key) => {
          if (eachMessage.from === "bot") {
            ...
            return (
              <div key={key} className="rightBallon messageBallon">
                <div>
                  <b>{eachMessage.from}:</b> <span>{eachMessage.text}</span>
                </div>
              </div>
      ...
      <form onSubmit={sendMessage}>
        <Box
          sx={{ position: "fixed", bottom: 0, width: "100%", display: "flex" }}
        >
          <TextField
            onChange={(e) => {
